...
def load_config():
    default_config = {
        "api_key": "你的api_key填写在这里",
        "base_url": "兼容openai的url",
        "target_model": "模型名称",
        "intimacy": 0,
        "scale": 0.5,
        "pos_x": -1,
    ...
    try:
        with open(CONFIG_FILE, "r", encoding="utf-8") as f:
            ...
            if "intimacy" not in user_config:
                user_config["intimacy"] = 0
...
def stream_chat_with_romasha(user_text, interrupted_text, thread_obj):
    ...
    dynamic_system_prompt = f"{persona.ROMASHA_PROMPT}\n\n"
    ...
    dynamic_system_prompt += f"再次警告：绝不能创造上面三个列表以外的任何标签！"
    ...
    try:
        ...
        thread_obj.chunk_received.emit(f"[act_trouble] 抱歉，我的思绪陷入了混乱... {e}")